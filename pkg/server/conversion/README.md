...
- Resources that have provider-observed lifecycle detail can implement `ProviderProvisioningStatusRead()` to override the condition-derived provisioning status for live resources. This is intentionally narrow: the resource still owns storing the provider observation, while this package owns projecting it into shared API metadata.
- Deletion takes precedence for provisioning state. If a resource is being deleted, the public provisioning status is reported as `deprovisioning` immediately, even when a provider-observed override is present.
